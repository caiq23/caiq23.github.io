...
if request.form("tags")="" then 
response.write "<script>alert('关键词不能为空！');location='javascript:history.back(-1)';</script>"
response.end
end if
if request.form("tit")="" then 
response.write "<script>alert('简介不能为空！');location='javascript:history.back(-1)';</script>"
response.end
end if
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
                ...
                <td width=82%> 
    <input type=text id="tags" name="tags" size="40" />　<input type="button" onclick="lastIndexDemo();" value="提 取" />
                </td>
              </tr>
                <tr> 
                <td width=18% align=right height=30>简 介：</td>
                <td width=82%> 
                 <textarea name="tit" rows="3" cols="90"></textarea>
                </td>
              </tr>
                <tr> 
