...
new Marquee(
{
	...
	<div id="iright">
		...
		<div id="myTab1_Content0" style="clear:both;">
			<ul id="Artlist10num"><%If IsSqlDataBase=0 then%>
            <%Call ShowArticle(0,10,0,"",100,"no","Rnd(ID-timer())",0,1,1)%><%else%>
            <%Call ShowArticle(0,10,0,"",100,"no","Hits desc,ID desc",0,1,1)%><%end if%>
            </ul>
   		</div>
  		<div id="myTab1_Content1" class="none" style="clear:both;">
			<ul id="Artlist10num"><%If IsSqlDataBase=0 then%>
            <%Call ShowArticle(0,10,0,"",100,"datediff('d',DateAndTime,Now()) <= 30","Hits desc,ID desc",0,1,1)%><%else%>
            <%Call ShowArticle(0,10,0,"",100,"datediff(dd,DateAndTime,GetDate()) <= 30","DateAndTime asc,ID desc",0,1,1)%><%end if%>
            </ul>
		</div>
  		</div>
	</div>
