...
sql="select * from BBS_Reply where 1=1"
  ...
  if len(dtfrom)>0 then sql=sql&" and reply_time>='"& dtfrom &" 00:00:00'"
  if len(dtto)>0 then sql=sql&" and reply_time<='"& dtto &" 24:00:00'"
  if see_user_id>0 then sql=sql&" and topic_id in (select topic_id from BBS_Reply where user_id="& see_user_id &")"
  ...
  page_size=12
	...
	'统计数量
	sql_dl="select count(id) from BBS_Reply where 1=1"
	if see_user_id>0 then sql_dl=sql_dl&" and topic_id in (select topic_id from BBS_Reply where  user_id="& see_user_id &")"
	set rs_dl=bbsconn.execute(sql_dl)
	all_order_0=chk_num(rs_dl(0))
	
	sql_dl="select count(id) from BBS_Reply where topic_state=1"
	if see_user_id>0 then sql_dl=sql_dl&" and topic_id in (select topic_id from BBS_Reply where  user_id="& see_user_id &")"
	set rs_dl=bbsconn.execute(sql_dl)
	all_order_1=chk_num(rs_dl(0))
	
	sql_dl="select count(id) from BBS_Reply where topic_state=2"	
	if see_user_id>0 then sql_dl=sql_dl&" and topic_id in (select topic_id from BBS_Reply where  user_id="& see_user_id &")"
	set rs_dl=bbsconn.execute(sql_dl)
	all_order_2=chk_num(rs_dl(0))
	
	sql_dl="select count(id) from BBS_Reply where topic_state=3"	
	if see_user_id>0 then sql_dl=sql_dl&" and topic_id in (select topic_id from BBS_Reply where  user_id="& see_user_id &")"
	set rs_dl=bbsconn.execute(sql_dl)
...
<div class="table-topmenu" >
    ...
    <%if len(key_word)>0 or len(dtfrom)>0 or len(dtto)>0 or see_user_id>0 then%>
        ...
        <%else%>
            <a href="?see_user_id=<%=see_user_id%>&see_type=0" class="btn <%if see_type=0 then %>btn-gray<%else%>btn-gray2<%end if%>">全部回复(<%=all_order_0%>)</a>
...
if len(key_word)>0 or len(dtfrom)>0 or len(dtto)>0 or see_user_id>0 then
    ...
    <%if see_user_id>0 then%><%=chk_db("BBS_User","user_id",see_user_id,"user_name")%> 参与回复的<%end if%>
...
<table class="table table-bordered table-hover table-responsive">
   <thead>
    ...
    <th>状态</th>   
    <th>发表时间</th>
    <th style="text-align:left;">回复内容</th>
    <th>操作</th>
      </tr>
   </thead>
   <tbody>
...
<% if rs.eof then%>
  ...
  reply_content=RegExphtml(rs("reply_content"))
  hy_do=""
  select case topic_state
	  case 1
	  	hy_how="<font color=red>未审核</font>"		
		hy_do=hy_do&" <a class=""btn btn-success"" href=""../inc/admin_replay_check.asp?mydo=checked&modid="& replay_id &""" onClick=""return makesure('是否审核该帖子?');"" title=审核>审核</a>"				 
	  case 2
	  	hy_how="<font color=green>正常</font>"
		 hy_do=" <a class=""btn btn-info"" href=""../inc/admin_replay_check.asp?mydo=masking&modid="& replay_id &""" onClick=""return makesure('是否屏蔽该帖子?\n屏蔽后该帖子主题无法查看，但是回复内容还可以被他人查看');"" title=屏蔽>屏蔽</a>" 
	  case 3
	  	hy_how="<font color=blue>屏蔽中</font>"	  
		hy_do="<a class=""btn btn-success"" href=""../inc/admin_replay_check.asp?mydo=unmasking&modid="& replay_id &""" onClick=""return makesure('是否解除屏蔽?');"" title=解除屏蔽>恢复</a>" 
  end select
  hy_do=hy_do&" <a href=""../inc/admin_replay_check.asp?mydo=del_replay&modid="& replay_id &""" onClick=""return makesure('是否删除该回复?\n将会删除回复，不影响主题');"" class=""btn btn-danger"" title=删除回复>删回复</a>" 
  %>
  ...
  <tr>
    ...
    <td><%=hy_how%>&nbsp;</td>  
    <td><%=howlong(reply_time)%>&nbsp;</td>
    <td style="text-align:left; padding:0px;"><a href="../?p<%=topic_id%>.html" target="_blank" title="<%=reply_content%>"><%=left(reply_content,15)%></a>&nbsp;</td>
    <td><%=hy_do%></td>
  </tr>
  <%  
